...
class SAM2FromYOLO:
    def __init__(self):
        rospy.init_node("sam2_from_yolo_node", anonymous=True)
        ...
        rospy.Subscriber("/camera1/color/image_raw",Image,self.image_callback,queue_size=1,buff_size=2**24)
        rospy.Subscriber("/yolov9/bboxes1",BoundingBoxes,self.bbox_callback,queue_size=1)

        self.mask_pub = rospy.Publisher("/sam2/mask_image1", Image, queue_size=1)
        self.vis_pub = rospy.Publisher("/sam2/vis_image1",Image,queue_size=1)
